...
def test_forward_pass_first_order():
    ...
    # ------ Create a random brickwall circuit ------
    nlayers = 3
    layers, qubits_in_layer = brickwall_circuit.get_random_layers(nsites, nlayers, first_layer_odd=True, is_TI=True)
    Gs = jnp.asarray(util.flatten(layers))
    Gs_mat = util.get_matrices(Gs)
    qubits = util.flatten(qubits_in_layer)
    ...
    # ------ FoR implementation ------
    psis = brickwall_passes.forward_pass_first_order_cached_TI(
        ket0_padded, Gs, qubits, flat_idxs_fw, layer_ends_fw, 
        first_layer_increasing=True, truncation_dim=chi_max,
        rel_tol=rel_tol, abs_tol=abs_tol
        )

    psi_final = brickwall_passes.forward_pass_first_order_TI(
        ket0_padded, Gs, qubits, flat_idxs_fw, layer_ends_fw, 
        first_layer_increasing=True, truncation_dim=chi_max,
        rel_tol=rel_tol, abs_tol=abs_tol
        )
    ...
    def get_exact_result(psi_0):
        psi_1 = util.tensor_product([Gs_mat[0],I2]) @ psi_0
        psi_2 = util.tensor_product([I2, Gs_mat[0]]) @ psi_1
        psi_3 = util.tensor_product([I,Gs_mat[1],I]) @ psi_2 
        psi_4 = util.tensor_product([Gs_mat[2],I2]) @ psi_3
        psi_5 = util.tensor_product([I2]+[Gs_mat[2]]) @ psi_4
    ...
    print("\tLosses are the same: ", losses_are_the_same)
...
def test_forward_pass_second_order():
    ...
    # ------ Create a random brickwall circuit ------
    nlayers = 2
    G_layers, qubits_in_layer = brickwall_circuit.get_random_layers(nsites, nlayers, first_layer_odd=True, is_TI=True)
    ...
    # ------ FoR implementation ------
    psis, Dpsis = brickwall_passes.forward_pass_second_order_TI(
    ...
    psi_0 = mps.get_vector(ket0)
    def get_exact_result(psi_0):
        psi_1 = util.tensor_product([Gs_mat[0],I2,I2]) @ psi_0
        psi_2 = util.tensor_product([I2,Gs_mat[0],I2]) @ psi_1
        psi_3 = util.tensor_product([I2,I2,Gs_mat[0]]) @ psi_2 
        psi_4 = util.tensor_product([I,I2]+[Gs_mat[1]]+[I]) @ psi_3
        psi_5 = util.tensor_product([I]+[Gs_mat[1]] + [I2, I]) @ psi_4
        res = [psi_5, psi_4, psi_3, psi_2, psi_1, psi_0][::-1]
        return res
    ...
    eps = 1e-7
    psi_b0 = psis[1:]
    layers_eps = [G + eps*Z for G,Z in zip(Gs, Zs)]
    psi_b_eps = tn_methods.compute_all_intermediate_states_TI(
        layers_eps, qubits_in_layer, ket0_padded, increasing_merging_order=True, 
        truncation_dim=chi_max, rel_tol=rel_tol, abs_tol=abs_tol)[1:]
...
def test_compute_loss_gradient_F():
    ...
    # ------ JAX ------
    def f(Gs_mat):  # Frobenius
        res = 0
        for _ket0, _bra0 in zip(ket0_padded.copy(), bra0_padded.copy()):
            ket1 = util.tensor_product([Gs_mat[0]]*3) @ mps.get_vector(_ket0)
            ket2 = util.tensor_product([I]+[Gs_mat[1]]*2+[I]) @ ket1
            ket3 = util.tensor_product([Gs_mat[2]]*3) @ ket2
            res += jnp.dot(mps.get_vector(_bra0), ket3).real
        return 1 - res/nbatch
    grad_f = jax.grad(f, holomorphic=False)
    cost_jax = f(Gs_mat)
    grad_jax = grad_f(Gs_mat).conj()

    # ------ TN methods (FoR) ------
    cost_F, grad_F = objective_function.compute_loss_gradient_F(
        ket0_padded, bra0_padded, Gs, qubits, first_layer_increasing=True, 
        truncation_dim=chi_max, rel_tol=rel_tol, abs_tol=abs_tol, is_TI=True)
...
def test_compute_loss_gradient_hvp_F():
    ...
    nlayers = 3
    G_layers, qubits_in_layer = brickwall_circuit.get_random_layers(nsites, nlayers, first_layer_odd=True, is_TI=True)
    Z_layers, _ = brickwall_circuit.get_random_layers(nsites, nlayers, first_layer_odd=True, is_TI=True)
    Gs = jnp.asarray(util.flatten(G_layers))
    Zs = jnp.asarray(util.flatten(Z_layers))
    qubits = util.flatten(qubits_in_layer)
...
def test_compute_loss_HST():
    ...
    def f(gates):  # Overlap
        ...
        for _ket0, _bra0 in zip(ket0_padded, bra0_padded):
            ket1 = util.tensor_product([gates[0]]*3) @ mps.get_vector(_ket0)
            ket2 = util.tensor_product([I]+[gates[1]]*2+[I]) @ ket1
            ket3 = util.tensor_product([gates[2]]*3) @ ket2
...
def test_compute_loss_gradient_hvp_HST_all_functions():
    ...
    cost_HS1 = objective_function.compute_loss_HST(
        ket0_padded, bra0_padded, Gs, qubits, True,  
        chi_max, rel_tol, abs_tol, is_TI=True
        )
...
def test_compute_riemannian_loss_gradient_hvp_HST_all_functions():
    ...
    cost_HS2, grad_HS2 = objective_function.compute_riemannian_loss_gradient_HST(
        ket0_padded, bra0_padded, Gs, qubits, True, chi_max, rel_tol, abs_tol, is_TI=True)
            
    # ------ loss, gradient, hvp ------
    cost_HS3, grad_HS3, hvp_HS3 = objective_function.compute_riemannian_loss_gradient_hvp_HST(
        ket0_padded, bra0_padded, Gs, Zs, qubits, True, chi_max, rel_tol, abs_tol, is_TI=True)
    ...
    # ------ Test looping versions ------
    cost_loop = objective_function.compute_loss_HST_loop(
        ket0_padded, bra0_padded, Gs, qubits, True, 
        chi_max, rel_tol, abs_tol, is_TI=True
        )
    cost_loop.block_until_ready()
    costs_match = jnp.allclose(cost_HS1, cost_loop)

    print("Looped cost matches: ", costs_match)
    assert costs_match
    
    cost_grad_loop, grad_loop = objective_function.compute_riemannian_loss_gradient_HST_loop(
        ket0_padded, bra0_padded, Gs, qubits, True, chi_max, rel_tol, abs_tol, is_TI=True)
    cost_grad_loop.block_until_ready()
    costs_match = jnp.allclose(cost_HS2, cost_grad_loop)
    grads_match = jnp.allclose(grad_HS2, grad_loop)

    print("Looped cost+grad matches cost: ", costs_match)
    print("Looped cost+grad matches grad: ", grads_match)
    assert costs_match
    assert grads_match
    
    cost_hvp_loop, grad_hvp_loop, hvp_loop = objective_function.compute_riemannian_loss_gradient_hvp_HST_loop(
        ket0_padded, bra0_padded, Gs, Zs, qubits, True, chi_max, rel_tol, abs_tol, is_TI=True)
    cost_hvp_loop.block_until_ready()
    costs_match = jnp.allclose(cost_HS3, cost_hvp_loop)
    grads_match = jnp.allclose(grad_HS3, grad_hvp_loop)
    hvps_match = jnp.allclose(hvp_HS3, hvp_loop)
    print("Looped cost+grad+hvp matches cost: ", costs_match)
    print("Looped cost+grad+hvp matches grad: ", grads_match)
    print("Looped cost+grad+hvp matches hvp: ", hvps_match)
    assert costs_match
    assert grads_match
    assert hvps_match
...
def test_riemannian_projection():
    ...
    cost_HS, grad_HS, hvp_HS = objective_function.compute_riemannian_loss_gradient_hvp_HST(
        ket0_padded, bra0_padded, Gs, Zs, qubits, first_layer_increasing=True, truncation_dim=chi_max, 
        rel_tol=rel_tol, abs_tol=abs_tol, is_TI=True
        )
